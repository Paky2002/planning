...
class BSTable {
  ...
  constructor(tableId, options) {
    ...
    var defaults = {
      ...
      onAdd: function() {},
      onReset:function() {},           // Called when added a new row
      ...
      advancedReset: {                     // Do not override advanced unless you know what youre doing
        columnLabel: 'Actions',
        buttonHTML: `<div class="btn-group pull-right">
              <button id="bEdit" type="button" class="btn btn-sm btn-default">
                  <span class="fa fa-edit" > </span>
              </button>
              <button id="bRes" type="button" class="btn btn-sm btn-default">
    ...
    /** @private */ this.actionsColumnHTMLReset = '<td name="bstable-actions">' + this.options.advancedReset.buttonHTML + '</td>'; 
  }
  ...
  init() {
    ...
    {
    this.table.find('thead tr').append('<th name="bstable-actions">' + this.options.advancedReset.columnLabel + '</th>');  // Append column to header
    this.table.find('tbody tr').append(this.actionsColumnHTMLReset);
    }
    else
    {
  ...
  aggiungiCamera (info) {
    ...
    if ($allRows.length==0) { // there are no rows. we must create them
      var newColumnHTML = '<td>' + info[0] + '</td><td>'+info[1]+'</td>'+this.actionsColumnHTML;
      this.table.find('tbody').append('<tr data-camera = "'+info[2]+'">'+newColumnHTML+'</tr>');
      ...
      $lastRow.attr('data-camera', info[2]);
      ...
      $cols.each(function(index) {
        ...
        if ($(column).attr('name')=='bstable-actions') {
          ...
          $(column).html(info[index]);  // clear the text
        }
      });
    }
  }
  ...
  aggiungiExtra (info) {
    var $allRows = this.table.find('tbody tr');
    if ($allRows.length==0) { // there are no rows. we must create them
      var newColumnHTML = '<td>' + info[0] + '</td><td>'+info[1]+'</td><td>'+info[2]+'</td>'+this.actionsColumnHTML;
      this.table.find('tbody').append('<tr data-camera = "'+info[2]+'">'+newColumnHTML+'</tr>');
    } else { // there are rows in the table. We will clone the last row
      var $lastRow = this.table.find('tr:last');
      $lastRow.clone().appendTo($lastRow.parent());  
  ...
  _resetEachColumn($editableColumns, $cols, howToModify) {
    // Go through each editable field and perform the howToModifyFunction function
      var n = 0;
      $cols.each(function() {
        n++;
  ...
  _addOnClickEventsToActions() {
    ...
    this.table.find('tbody tr #bRes').each(function() {let button = this; button.onclick = function() {_this._rowReset(button)} });
